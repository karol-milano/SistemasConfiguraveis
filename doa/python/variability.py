...
class Variability:
	...
	def ownership_rlm (self, qtd_commits_ac, ownership_n, qtd_commits_dl):
		"""
	...
	def ownership_arquivo_perc (self, rows):
		"""
	...
	def parse_variability(self, json_data):
		rows = []
		...
		for varname, vardata in json_data['Variabilidades'].items():
			...
			for authorname, authordata in vardata['Autores'].items():
				...
				for filename in authordata['Arquivos']:
					...
					for commit in authordata['Commits']:
						id_commit = self.str_split(commit)
			...
			for r in vrows:
				...
				try:
					r['ownership_rlm'] = self.ownership_rlm(r['qtd_commits_ac'], r['ownership_n'], r['qtd_commits_dl'])
				except Exception as err:
					print("Exception: ", err)
		...
		return self.ownership_arquivo_perc(rows)
