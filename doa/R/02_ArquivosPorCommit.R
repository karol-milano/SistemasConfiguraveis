...
gerarArquivosPorCommit <- function(projeto, valores) {
  ...
  tabela <- paste("../resultados/", projeto, "/02_ArquivosPorCommit.csv", sep = "")
  
  write.table(arq_var, file=tabela, sep=",", row.names=F)
  
  
  imagem <- paste("../resultados/", projeto, "/02_ArquivosPorCommit.png", sep = "")
  ...
  p <- arq_var %>%
    ...
    labs(x = "Commits", y = "Quantidade de Arquivos",
         title = "Quantidade de arquivos por commit"
    )
